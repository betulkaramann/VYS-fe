            ...
            return (
              <div key={category}>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-bold text-gray-900">₺{categoryValue.toLocaleString('tr-TR')} ({categoryPercentage.toFixed(1)}%)</span>
                </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="space-y-2">
          {materials.filter(m => m.quantity < m.minStock).map((material) => (
            <div key={material.id} className="flex items-center justify-between p-3 border border-yellow-200 rounded-lg bg-yellow-50">
              <div>
                <p className="font-medium text-gray-900">{material.name}</p>
                <p className="text-sm text-gray-600">{material.quantity} / {material.minStock} ({material.warehouse})</p>
              </div>
              <button className="px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 text-sm font-medium">
                Sipariş Ver
              </button>
            </div>
          ))}
          ...
          <div key={warehouse} className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-gray-600 text-sm mt-1">{warehouseMaterials.length} farklı malzeme • Toplam: {warehouseQuantity} adet</p>
              </div>
              <span className="text-2xl font-bold text-green-600">₺{warehouseValue.toLocaleString('tr-TR')}</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {warehouseMaterials.map((material) => (
                <div key={material.id} className="border border-gray-200 rounded-lg p-3 hover:bg-gray-50 transition">
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <span className="text-sm font-bold text-blue-600">{material.quantity} {material.unit}</span>
                    <span className="text-xs text-gray-600">₺{(material.quantity * material.unitPrice).toLocaleString('tr-TR')}</span>
                  </div>
                </div>
              ))}
            </div>
